 ...
 * @type string
 * @desc Legacy setting. Menu normal text now follows RPG Maker MZ Color 0 (\\C[0]).
 * @default #ffffff
 *
...
(() => {
  ...
  // Normal text color fix:
  // Keep RPG Maker MZ's native ColorManager.normalColor() unchanged.
  // This makes menu text use the project's normal text color (\\C[0])
  // and its normal outline instead of forcing ThemeTextColor globally.
  ...
  // === DryPromise final QA: readable main-menu text ===
  // Keep this limited to the main command window. Other option/save/system text is untouched.
  const _DPQA_WMC_drawItem = Window_MenuCommand.prototype.drawItem;
  Window_MenuCommand.prototype.drawItem = function(index) {
    const symbol = this.commandSymbol(index);
    if (cfg.menuIcons && cfg.menuIcons[symbol]) {
      _DPQA_WMC_drawItem.call(this, index);
      return;
    }
    ...
    // Gold focus box -> dark text. Dark inactive rows -> warm ivory text.
    this.contents.textColor = isSelected ? "#2A211B" : "#F3E8D2";
