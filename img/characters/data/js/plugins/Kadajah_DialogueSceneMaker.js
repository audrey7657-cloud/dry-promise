...
(() => {
    ...
    function parseScript(text) {
        ...
        const commandRegex = /^\[(WAIT|PAUSE|SE|CE|SWITCH|VAR|BGM|BGS|ME|SCRIPT|LABEL|GOTO|CHOICE|BUST|HIDE_BUSTS|FADEOUT|FADEIN|TINT|SHAKE|FLASH|WEATHER|MOVE_TO|MOVE|TURN|JUMP|ANIM|OPACITY|STEP_ANIME|TELEPORT|SPEED|FREQ|WALK_ANIME|DIR_FIX|THROUGH|TRANSPARENT|SCROLL|CAMERA_RESET)\s*(.*)\]$/i;
    ...
    function _spBustShow(resolved) {
        ...
        const bustFile = imageMap[String(resolved.faceIndex)];
        if (!bustFile) return;
    ...
    Game_Interpreter.prototype.command101 = function(params) {
        ...
        if (resolved) {
            ...
            // 원래 Face Index는 큰 일러스트 감정 선택에 그대로 사용한다.
            _spBustShow(resolved);

            // 다만 RPG Maker 기본 메시지창의 왼쪽 작은 얼굴만 숨긴다.
            const messageParams = params.slice();
            messageParams[0] = "";
            return _spBust_GameInterpreter_command101.call(this, messageParams);
